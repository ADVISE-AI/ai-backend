...
def user_input_builder(clean_data: dict) -> dict:
    ...
    # Extract timestamp from clean_data (Unix timestamp in seconds)
    timestamp = None
    if "timestamp" in clean_data:
        try:
            timestamp = datetime.fromtimestamp(int(clean_data["timestamp"]))
        except (ValueError, TypeError) as e:
            _logger.warning(f"Failed to parse timestamp {clean_data.get('timestamp')}: {e}")
    
    if clean_data["class"] == "text":
        if clean_data["context"]:
                with engine.begin() as conn:
                    result = conn.execute(select(message.c.message_text, message.c.media_info, message.c.provider_ts).where(message.c.external_id == clean_data["context"]["id"]))
                    row = result.mappings().first()
                if row and row["media_info"]:
                    media_info = json.loads(row["media_info"])
                    # Use provider_ts from database for context media
                    media_timestamp = row.get("provider_ts")
                    downloaded_data =  download_media(media_info["id"], timestamp=media_timestamp)
                    ...
                    if downloaded_data:
                        ...
                        _logger.warning(f"Failed to download context media {media_info['id']}, falling back to text-only")
                        user_input = {
                            "context": False,
                            "class": "text",
                            "message": clean_data["from"]["message"]
        ...
        downloaded_data =  download_media(clean_data["from"]["media_id"], timestamp=timestamp)
        ...
        if downloaded_data:
            ...
            _logger.error(f"Failed to download media {clean_data['from']['media_id']}, cannot process")
            user_input = {
                "context": False,
                "class": "text",
                "message": "Sorry, I couldn't process the media you sent."
            }
